 ...
 * @Description: TODO随机数的处理类
 * @author: 曹少帅·
 * @date: 2019年7月27日 上午9:30:33
 */
public class RandomUitl {
	...
	//方法4：返回参数length个字符(5分)，方法内部要调用randomCharacter()方法 (4分)
	...
	//：返回1个a-z之间的随机字符。 (8分)
	public static char randomEnglishchar(){
		//穷举1-9,a-Z的字符
		String str = "qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM";
	//	charAt(index).根据下标返回字符串的值
		return str.charAt(random(0, str.length()-1));
	}
	//返回参数length个字符，方法内部要调用randomCharacter()方法 (4分)
	public static String randomEnglishLen(int length){
		StringBuilder sb = new StringBuilder();
		for(int i=0;i<length;i++) {
			sb.append(randomEnglishchar());
		}
		return sb.toString();
	}
